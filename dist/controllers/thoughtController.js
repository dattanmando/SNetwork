...
const getThoughtById = async (req, res) => {
    try {
        const thought = await Thoughts_1.default.findById(req.params.id);
        if (!thought) {
...
const createThought = async (req, res) => {
    try {
        ...
        const thought = await Thoughts_1.default.create({ thoughtText, username });
...
const updateThought = async (req, res) => {
    try {
        const thought = await Thoughts_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!thought) {
            res.status(404).json({ message: 'Thought not found' });
            return;
        }
...
const deleteThought = async (req, res) => {
    try {
        const thought = await Thoughts_1.default.findByIdAndDelete(req.params.id);
        if (!thought) {
            res.status(404).json({ message: 'Thought not found' });
            return;
        }
